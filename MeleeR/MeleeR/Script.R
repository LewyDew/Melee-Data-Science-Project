...
df$p2_cstick_x <- cut(df$p2_cstick_x, breaks = 40)
df$p2_cstick_y <- cut(df$p2_cstick_y, breaks = 40)
df$p2_joystick_x <- cut(df$p2_joystick_x, breaks = 40)
df$p2_joystick_y <- cut(df$p2_joystick_y, breaks = 40)
df$p2_triggers <- cut(df$p2_triggers, breaks = 40)
...
df$p2_position_x <- round(df$p2_position_x, digits = 0)
df$p2_position_y <- round(df$p2_position_y, digits = 0)
df$p1_damage <- round(df$p1_damage, digits =0)
df$p1_shield <- round(df$p1_shield,digits = 0)
df$p2_damage <- round(df$p2_damage,digits =0)
df$p2_shield <- round(df$p2_shield,digits =0)
...
df["next_button_p2"] <- lead(df$p2_buttons)
#str(df)
...
rf1 <- h2o.randomForest(## h2o.randomForest function
  ...
  x = 1:35, ## the predictor columns, by column index
